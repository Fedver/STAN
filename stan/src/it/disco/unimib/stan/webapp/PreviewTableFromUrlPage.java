...
public class PreviewTableFromUrlPage implements ApplicationHandler {

	@Override
	public String route() {
		return "preview-table-url";
	}
	...
	public String process(Communication requestAndResponse) throws Exception {
		...
		if(parameterValidator.isCorrectRequest(requestAndResponse)){
			...
			try{
				...
				requestAndResponse.sendRedirect("/preview", "info", tableName + " successfully uploaded");
			}
			catch(Exception e){
				requestAndResponse.sendRedirect("/preview", "error", "Error importing table");
			}
		}
		else{
			requestAndResponse.sendRedirect("/preview", "error", "Error importing table");
